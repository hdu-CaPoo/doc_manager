...
def upload_document(
    ...
    if existing_doc:
        ...
        if existing_doc.file_hash == new_hash:
            raise HTTPException(
                status_code=400, detail="文件已存在且内容未变，请勿重复提交"
            )
        ...
        if not overwrite:
            ...
            raise HTTPException(
                status_code=409, detail=f"文件 '{file.filename}' 已存在。是否覆盖？"
            )
...
def delete_document(
    ...
    if not (
        doc.uploader_id == current_user.id or doc.project.owner_id == current_user.id
    ):
...
def restore_document(
    ...
    doc = (
        db.query(Document)
        .filter(Document.id == document_id, Document.is_deleted == True)
        .first()
    )
    if not doc:
...
def list_trashed_documents(
    ...
    trashed_docs = (
        db.query(Document)
        .filter(Document.project_id == project_id, Document.is_deleted == True)
        .all()
    )
