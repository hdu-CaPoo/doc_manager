...
def update_existing_issue(
    ...
    updated_issue = update_issue(
        db, db_issue=issue, new_status=new_status, new_content=new_content
    )
...
def delete_issue_endpoint(
    ...
    member = get_member(db, project_id, current_user.id)
    if not member or (
        member.role not in ["admin", "owner"] and issue.creator_id != current_user.id
    ):
        raise HTTPException(
            status_code=403, detail="你不是该项目管理员或拥有者或提问者本人"
        )
...
# --- 新增：发表回复 ---
@router.post(
    "/projects/{project_id}/issues/{issue_id}/comments", response_model=CommentOut
)
def reply_to_issue(
    project_id: int,
    issue_id: int,
    ...
    if current_user.id != issue.creator_id:
        ...
        email_content = (
            f"{current_user.nickname} 回复了你：{comment_in.content[:50]}..."
        )
    ...
    return create_comment(
        db, issue_id=issue_id, user_id=current_user.id, content=comment_in.content
    )
...
@router.get(
    "/projects/{project_id}/issues/{issue_id}/comments", response_model=List[CommentOut]
)
