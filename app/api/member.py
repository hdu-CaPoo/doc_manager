...
def kick_member(
    ...
    if not (project.owner_id == current_user.id or get_member(db, project_id, current_user.id).role == "admin"):
        raise HTTPException(status_code=403, detail="只有项目管理员可以移除成员")
    
    # c. 不能踢自己 (防止项目变成没管理员的孤儿)
    if user_id == current_user.id:
        raise HTTPException(status_code=400, detail="不能移除你自己，请使用退出项目功能")
...
def leave_project(
    ...
    if project.owner_id == current_user.id:
        raise HTTPException(status_code=400, detail="项目拥有者不能直接退出项目，请先转让拥有者权限或删除项目")
