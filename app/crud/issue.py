...
def get_issues_by_project(db: Session, project_id: int, status: str = None):
    if status:
        return (
            db.query(Issue)
            .filter(Issue.project_id == project_id, Issue.status == status)
            .all()
        )
    else:
...
# 3. 更新提问状态或内容
def update_issue(
    db: Session, db_issue: Issue, new_status: str = None, new_content: str = None
):
    if new_status:
...
def create_comment(db: Session, issue_id: int, user_id: int, content: str):
    db_comment = IssueComment(issue_id=issue_id, user_id=user_id, content=content)
    db.add(db_comment)
    db.commit()
